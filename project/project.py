...
class RGBImage:
    ...
    """
    #use copy.deepcopy(arg) for pointer issues with mutable items
    ...
    def __init__(self, pixels):
        ...
        if isinstance(pixels, list) and len(pixels) > 0:
            ...
            if all([isinstance(row, list) and len(row) > 0 and row_len == len(row)for row in pixels]):
                if all([all([isinstance(ele, list) and len(ele) == 3 for ele in row]) for row in pixels]): #col check
                    terror = False
                    if all([ isinstance(pix, int) and pix >= 0 and pix <= 255 for row in pixels for ele in row for pix in ele]):
                        verror = False
    ...
    def get_pixel(self, row, col):
        ...
        if row < 0 or col < 0 or row > self.num_rows -1 or col > self.num_cols -1:
            raise ValueError()
        return tuple(self.get_pixels()[row][col])
    ...
    def set_pixel(self, row, col, new_color):
        ...
        """
        # YOUR CODE GOES HERE #
        #dont want to make a copy because we want to actually update here
        #only modify the color channels with valid inputs
        if not isinstance(row, int) or not isinstance(col, int):
            raise TypeError()
        if row < 0 or col < 0 or row > self.num_rows -1 or col > self.num_cols -1:
            raise ValueError()
        if not isinstance(new_color, tuple) and len(new_color) == 3 and all(isinstance(ele, int) for ele in new_color): # is is for memory
            raise TypeError()
        if not all(225 >= pix for pix in new_color):
            raise ValueError()
